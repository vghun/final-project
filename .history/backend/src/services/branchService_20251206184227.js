...
export const createBranch = async (data) => {
  try {
    ...
    // 1️⃣ Tạo chi nhánh, map startDate → resumeDate
    const newBranch = await Branch.create({
      ...
      slotDuration,
      status: "Active",
      resumeDate: startDate || new Date(), // ngày bắt đầu hoạt động
    });

    // 2️⃣ Gán dịch vụ nếu có selectedServices
    if (Array.isArray(selectedServices) && selectedServices.length > 0) {
      await newBranch.addServices(selectedServices);
    }

    // 3️⃣ Lấy lại full thông tin chi nhánh + dịch vụ
