...
const setSuspendDate = async (branchId, suspendDate) => {
  try {
    ...
    if (!suspend) throw new Error("Ngày tạm ngưng không hợp lệ!");
      const today = new Date();
    today.setHours(0, 0, 0, 0); // chỉ so sánh ngày, bỏ giờ
    suspend.setHours(0, 0, 0, 0);

    if (suspend <= today) {
      throw new Error("Ngày tạm ngưng phải lớn hơn ngày hôm nay ít nhất 1 ngày!");
    }
    ...
    return {
      success: true,
      hasBooking: false,
      message: "Cài đặt ngày tạm ngưng chi nhánh thành công!",
    };
  } catch (error) {
