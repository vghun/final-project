...
export const createService = async (req, res) => {
  try {
    let { name, description, price, duration, branches } = req.body;

    // Parse branches từ string JSON
    let branchIds = [];
    try {
      branchIds = branches ? JSON.parse(branches) : [];
      if (!Array.isArray(branchIds)) branchIds = [];
    } catch (err) {
      return res.status(400).json({ success: false, message: "branches phải là mảng JSON" });
    }
    ...
    if (!name) return res.status(400).json({ success: false, message: "Tên dịch vụ không được để trống" });
    if (isNaN(price) || price <= 0) return res.status(400).json({ success: false, message: "Giá dịch vụ phải là số lớn hơn 0" });
    if (isNaN(duration) || duration <= 0) return res.status(400).json({ success: false, message: "Thời lượng phải là số nguyên lớn hơn 0" });
    ...
    const service = await serviceService.createService(serviceData, branchIds);
