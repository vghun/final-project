...
const fetchData = async (m, y) => {
  ...
  try {
    ...
    // Lấy monthData từ overview
    const monthData = overviewData.find(item => item.month === m && item.year === y);

    // Lấy dữ liệu real-time
    const realTimeData = await SalaryAPI.getSalaries(m, y);

    // Nếu đã có dữ liệu lưu, lấy status từ đó
    const salaryData = realTimeData.map(rt => {
      const saved = monthData?.salaries.find(s => s.idBarber === rt.idBarber);
      return {
        ...rt,
        status: saved ? saved.status : "Chưa tính",
      };
    });
  ...
  // Kiểm tra nút Tính lương: chỉ enable khi canCalculate = true
