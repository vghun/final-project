...
function LuongThuong() {
  ...
  const [overview, setOverview] = useState([]); // Lưu trạng thái tháng trước
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(false);

  // Load danh sách tháng + trạng thái lương
  ...
  // Load bảng lương cho tháng chọn
  const loadSalaries = useCallback(async () => {
    ...
    try {
      ...
      if (isCurrentMonth) {
        // Tháng hiện tại → realtime
        data = await SalaryAPI.getSalaries(month, year);
      } else {
        // Tháng trước → lấy từ overview
        const monthData = overview.find(m => m.month === month && m.year === year);
        if (monthData) {
          data = monthData.salaries;
        } else {
          // Nếu chưa có overview → lấy realtime snapshot + status chưa tính
          const snapshot = await SalaryAPI.getSalaries(month, year);
          data = snapshot.map(s => ({ ...s, status: "Chưa tính" }));
        }
      }
  ...
  const now = new Date();
  const isCurrentMonth = month === now.getMonth() + 1 && year === now.getFullYear();
  const canCalculate = !isCurrentMonth && salaries.some(s => s.status !== "Đã tính");
              ...
              {filteredSalaries.length > 0 ? (
                filteredSalaries.map((s, idx) => (
                  <tr key={idx}>
                    ...
                    <td>
                      <span
                        className={cx("status", s.status === "Đã tính" ? "calculated" : "pending")}
                      >
                        {s.status}
                      </span>
                    </td>
