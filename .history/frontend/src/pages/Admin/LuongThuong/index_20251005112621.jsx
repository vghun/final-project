...
function LuongThuong() {
  ...
  const fetchData = async (m, y) => {
    ...
    try {
      ...
      const statusMap = {};
      if (monthData) {
        monthData.salaries.forEach(s => statusMap[s.idBarber] = s.status === "Đã tính");
      }
  ...
  // Load khi mount và khi đổi tháng/năm
  useEffect(() => {
  ...
  const calculateSalary = async () => {
    ...
    try {
      ...
      await fetchData(month, year); // reload dữ liệu
    } catch (error) {
  ...
  // Kiểm tra trạng thái nút "Tính lương"
    ...
    <div className={cx("salaryPage")}>
      ...
      <div className={cx("salaryTable")}>
        ...
        {loading ? <p>Đang tải dữ liệu...</p> : (
          <div className={cx("tableWrapper")}> {/* thêm wrapper scroll */}
            <table>
              <thead>
                <tr>
                  <th>Thợ cắt tóc</th>
