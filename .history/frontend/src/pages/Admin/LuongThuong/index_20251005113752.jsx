...
function LuongThuong() {
  ...
  const calculateSalary = async () => {
    ...
    try {
      ...
      // Cập nhật overview để nút không hiển thị nữa
      setOverview(prev => {
  ...
  // Kiểm tra nút Tính lương: chỉ hiện cho tháng đã qua và chưa tính
  const now = new Date();
  const isPastMonth = year < now.getFullYear() || (year === now.getFullYear() && month < now.getMonth() + 1);
  const monthOverview = overview.find(item => item.month === month && item.year === year);
  const canCalculate = isPastMonth && (!monthOverview || monthOverview.canCalculate !== false);
