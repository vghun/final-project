...
function LuongThuong() {
  ...
  // Lấy dữ liệu overview + bảng lương
  const fetchData = async (m, y) => {
    ...
    try {
      ...
      let salaryData = [];
      if (monthData) {
        // Lấy đúng dữ liệu từng thợ từ server
        salaryData = monthData.salaries.map(s => ({
          idBarber: s.idBarber || null,
          barberName: s.barberName || "Chưa có tên",
          branchName: s.branchName || "Chưa có chi nhánh",
          baseSalary: s.baseSalary || 0,
          commission: s.commission || 0,
          tip: s.tip || 0,
          bonus: s.bonus || 0,
          totalSalary: s.totalSalary || 0,
          status: s.status || "Chưa tính",
          serviceRevenue: s.serviceRevenue || 0,
        }));
      } else {
        // Nếu tháng chưa có dữ liệu → mặc định "Chưa tính"
        const rawSalaries = await SalaryAPI.getSalaries(m, y);
        salaryData = rawSalaries.map(s => ({
          idBarber: s.idBarber || null,
          barberName: s.barberName || "Chưa có tên",
          branchName: s.branchName || "Chưa có chi nhánh",
          baseSalary: s.baseSalary || 0,
  ...
  const calculateSalary = async () => {
    ...
    try {
      ...
      // Cập nhật trực tiếp status từng thợ
      setSalaries(prev => prev.map(s => ({ ...s, status: "Đã tính" })));

      // Cập nhật overview để disable nút
      setOverview(prev => {
  ...
  // Nút tính lương enable khi canCalculate = true
  const monthOverview = overview.find(item => item.month === month && item.year === year);
  const canClick = monthOverview ? monthOverview.canCalculate : false;
