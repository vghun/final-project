...
const loadSalaries = useCallback(async () => {
  setLoading(true);
  try {
    const data = await SalaryAPI.getSalaries(month, year); // luôn realtime
    // Nếu là tháng đã tính thì override status theo overview
  ...
  // Tính lương toàn bộ thợ
  const calculateSalary = async () => {
    setLoading(true);
    try {
      await SalaryAPI.calculateSalaries(month, year); // chỉ lưu xuống DB
      alert(`Đã tính lương cho tháng ${month}/${year}`);
      await loadOverview(); // cập nhật trạng thái
      loadSalaries();       // load bảng lương mới
    } catch (error) {
