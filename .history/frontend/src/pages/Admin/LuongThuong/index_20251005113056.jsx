...
function LuongThuong() {
  ...
  const fetchData = async (m, y) => {
    ...
    try {
      ...
      // Tạo map trạng thái nhanh
      const monthData = overviewData.find(item => item.month === m && item.year === y);
      const statusMap = new Map();
      if (monthData?.salaries?.length) {
        monthData.salaries.forEach(b => statusMap.set(b.idBarber, b.status));
      }

      // Gán status cho từng thợ
  ...
  const calculateSalary = async () => {
    ...
    try {
      ...
      await fetchData(month, year); // reload dữ liệu ngay sau khi tính
    } catch (error) {
