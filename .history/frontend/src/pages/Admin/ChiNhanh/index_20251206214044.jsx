...
function ChiNhanh() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editingBranch) await BranchAPI.update(editingBranch.id, formData);
      else await BranchAPI.create(formData);

      showToast("success", editingBranch ? "Cập nhật chi nhánh thành công!" : "Thêm chi nhánh thành công!");
      setShowForm(false);
      fetchBranches();
    } catch (err) {
      console.error(err);
