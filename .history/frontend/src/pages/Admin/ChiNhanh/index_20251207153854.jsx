...
const openSuspendForm = (branch, mode) => {
  ...
  setSuspendMode(mode); // 'suspend' hoặc 'resume'
  ...
  // 1. Active, chưa tạm ngưng
  if (branch.status === "Hoạt động" && !branch.suspendDate) {
    openSuspendForm(branch, "suspend"); // Mở form để đặt suspendDate
    return;
  }

  // 2. Active, đã set suspendDate nhưng chưa tới ngày
  ...
  // 3. Inactive, đang tạm ngưng, cần set resumeDate
  if (branch.status === "Tạm ngưng" && (!branch.resumeDate || branch.resumeDate > today)) {
    openSuspendForm(branch, "resume"); // Mở form chỉ chỉnh resumeDate
    return;
  }

  // 4. Inactive, đã set resumeDate nhưng chưa tới ngày
  if (branch.status === "Tạm ngưng" && branch.resumeDate && branch.resumeDate > today) {
    showToast(
      "info",
  ...
  // 5. Sau ngày resumeDate → Active, chỉ xem lịch sử
