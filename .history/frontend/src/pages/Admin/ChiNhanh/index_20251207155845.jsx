...
setBranches(
  list.map((b) => ({
    ...
    suspendInfo: {
      isSuspended: b.status === "Inactive" && b.resumeDate && b.resumeDate > new Date().toISOString().split("T")[0],
      alreadySet: !!b.suspendDate || !!b.resumeDate,
...
const openEditForm = (branch) => {
  ...
  setFormData({
    ...
    isEditable: branch.status === "Inactive", // check backend status
  ...
  // branch.status là giá trị backend: Active / Inactive
  if (branch.status === "Active") {
    // chưa có ngày suspend thì mở modal tạm ngưng
    if (!branch.suspendDate) {
      openSuspendForm(branch, "suspend");
      return;
    }

    // có suspendDate nhưng chưa tới ngày thì thông báo
    if (branch.suspendDate > today) {
      showToast(
        "info",
  ...
  if (branch.status === "Inactive") {
    // chưa có resumeDate hoặc ngày resume còn tương lai thì mở modal kích hoạt
    if (!branch.resumeDate || branch.resumeDate > today) {
      openSuspendForm(branch, "resume");
      return;
    }

    // resumeDate đã tới, thông báo chi nhánh đã hoạt động trở lại
    if (branch.resumeDate && branch.resumeDate <= today) {
      showToast(
        "info",
