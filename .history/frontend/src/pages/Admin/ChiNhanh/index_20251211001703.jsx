  ...
  const handleSuspendSubmit = async (e) => {
    ...
    try {
      if (suspendMode === "suspend") {
        await BranchAPI.setSuspend(suspendBranch.id, { suspendDate });
      } else {
        await BranchAPI.setResume(suspendBranch.id, { resumeDate });
      }

showToast("success", res.message || "Đã lưu trạng thái chi nhánh!");
