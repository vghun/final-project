...
const handleAddressChange = (e) => {
  ...
  // Nếu người dùng gõ cấp huyện
  allDistricts.forEach((d) => {
    if (normalize(d.name).includes(keyword)) {
      const province = allProvinces.find((p) => p.code === d.province_code);
      results.push(`${d.name}, ${province?.name}`);
    }
  });

  // Nếu gõ cấp tỉnh
  allProvinces.forEach((p) => {
    if (normalize(p.name).includes(keyword)) {
      results.push(p.name);
    }
  });

  setSuggestions(results.slice(0, 10)); // giới hạn 10 gợi ý
};
...
const selectSuggestion = (fullAddress) => {
  ...
  {suggestions.length > 0 && (
    ...
    <div>
      <label>Quản lý</label>
      <input name="managerId" value={formData.managerId} onChange={handleChange} />
    </div>
  </div>
