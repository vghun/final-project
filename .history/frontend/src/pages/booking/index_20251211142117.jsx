...
if (!booking.services.length)
  ...
  {branches.map((b) => {
    ...
    // Nếu có ngày suspend nhưng không có resume → chi nhánh đang ngưng
    if (suspend && !resume) {
      label += ` — (ngưng hoạt động từ: ${suspend})`;
    }

    // Nếu có resumeDate và resumeDate > hôm nay → đang chờ hoạt động lại
    if (resume && new Date(b.resumeDate) > new Date()) {
      label += ` — (hoạt động lại từ: ${resume})`;
    }
