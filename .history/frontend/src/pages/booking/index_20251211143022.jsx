...
function BookingPage() {
  ...
  const handleBranchChange = async (e) => {
    ...
    const selected = branches.find(b => b.idBranch === id);
    setBooking({ ...booking, branchId, branch: selected ? selected.name : "", barber: "", barberId: null, services: [], time: "", date: "" });
...
>
  ...
  {branches.map((b) => {
    ...
    let optionLabel = b.name;

    if (suspend && !resume) {
      optionLabel += ` — (sẽ ngưng hoạt động từ ${suspend})`;
    }
    if (resume && new Date(b.resumeDate) > new Date()) {
      optionLabel += ` — (sẽ hoạt động từ ${resume})`;
    }
