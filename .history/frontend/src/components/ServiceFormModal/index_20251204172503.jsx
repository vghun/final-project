...
export default function ServiceFormModal({ show, onClose, serviceId, branches, onUpdated }) {
  ...
  const [editMode, setEditMode] = useState(true); // true = view only
  ...
  useEffect(() => {
    ...
    const loadDetail = async () => {
      ...
      setEditMode(true); // luôn view-only khi mở
  ...
  const handleEditClick = async () => {
    try {
      // kiểm tra lịch booking từ hôm nay trở đi
      const canEdit = await ServiceAPI.checkEditable(serviceId); 
      if (!canEdit) {
        alert("Dịch vụ này đang được đặt cho các lịch tương lai, không thể chỉnh sửa!");
        return;
      }

      // tạm ẩn dịch vụ trước khi sửa
      await ServiceAPI.updateStatus(serviceId, "Inactive");

      setEditMode(false); // cho phép chỉnh sửa
    } catch (error) {
      console.error(error);
    ...
    <div className={cx("overlay")}>
      <div className={cx("modal")}>
          ...
          <div className={cx("contentRow")}>
            ...
            <div className={cx("leftImage")}>
              <div className={cx("imagePreview")}>
                <img src={form.imageFile ? URL.createObjectURL(form.imageFile) : serviceDetail?.image} alt="service" />
              </div>
            ...
            <div className={cx("rightInfo")}>
              ...
              <div className={cx("statusToggle")} onClick={() => !editMode && setForm({...form, status: form.status === "Active" ? "Inactive" : "Active"})}>
        ...
        <div className={cx("footer")}>
          {editMode ? (
            <button className={cx("save")} onClick={handleEditClick}>Chỉnh sửa</button>
          ) : (
            <button className={cx("save")} onClick={handleSubmit}>Lưu thay đổi</button>
          )}
        </div>
      </div>
    </div>
