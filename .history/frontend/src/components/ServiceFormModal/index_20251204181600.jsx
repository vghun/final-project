...
const handleClose = () => {
    if (editMode) {
        const confirmClose = window.confirm("Bạn có chắc muốn tắt khi chưa cập nhật không?");
        if (!confirmClose) return; // Người dùng chọn Không → không đóng
    }
    onClose(); // Người dùng chọn Có → đóng form
    };
