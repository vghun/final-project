...
export default function ServiceFormModal({ show, onClose, serviceId, branches, onUpdated }) {
  ...
  const handleEditClick = async () => {
    try {
      const canEdit = await ServiceAPI.checkEditable(serviceId); // check lịch booking tương lai
      if (!canEdit) {
        setToast({ type: "error", text: "Dịch vụ này đang có booking tương lai, không thể chỉnh sửa!", duration: 3000 });
        return;
      }

      // Tạm ẩn dịch vụ trước khi chỉnh sửa
      await ServiceAPI.updateStatus(serviceId, "Inactive");

      setEditMode(true);
      setToast({ type: "success", text: "Dịch vụ đã tạm ẩn, bạn có thể chỉnh sửa.", duration: 3000 });
    } catch (error) {
      console.error(error);
