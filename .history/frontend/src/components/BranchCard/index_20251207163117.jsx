...
function BranchCard({
  ...
  if (isActive) {
    ...
    toggleText = `Tạm ngưng (sẽ bắt đầu từ ${new Date(suspendDate).toLocaleDateString()})`;
  } else if (resumeDate && resumeDate > today) {
    toggleText = `Đang tạm ngưng (sẽ hoạt động từ ${new Date(resumeDate).toLocaleDateString()})`;
  } else if (suspendDate && resumeDate) {
    toggleText = `Đang tạm ngưng (${new Date(suspendDate).toLocaleDateString()} - ${new Date(resumeDate).toLocaleDateString()})`;
  } else {
  ...
  if (isActive) {
    ...
    tooltipText = `Chi nhánh sẽ tạm dừng từ ${new Date(suspendDate).toLocaleDateString()}`;
  } else if (resumeDate && resumeDate > today) {
    tooltipText = `Chi nhánh đang tạm ngưng, sẽ hoạt động trở lại từ ${new Date(resumeDate).toLocaleDateString()}`;
  } else if (suspendDate && resumeDate) {
    tooltipText = `Chi nhánh đang tạm ngưng từ ${new Date(suspendDate).toLocaleDateString()} đến ${new Date(resumeDate).toLocaleDateString()}`;
  } else {
