...
function BranchCard({
  ...
  return (
    ...
    <button
  className={cx("toggleBtn", { off: !isActive, tooltip: suspendDate && suspendDate > new Date().toISOString().split("T")[0] })}
  onClick={onToggle}
  disabled={isSuspended && alreadySet && suspendDate > new Date().toISOString().split("T")[0]}
  data-tooltip={
    suspendDate && suspendDate > new Date().toISOString().split("T")[0]
      ? `Chi nhánh sẽ tạm dừng từ ${new Date(suspendDate).toLocaleDateString()}`
      : ""
  }
>
