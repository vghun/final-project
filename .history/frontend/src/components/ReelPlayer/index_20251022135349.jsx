...
useEffect(() => {
  ...
  let scrollTimeout = null; // hẹn giờ debounce
  let isLocked = false; // khóa cuộn trong thời gian animation

  const handleWheel = (e) => {
    e.preventDefault();

    // Nếu đang khóa thì bỏ qua toàn bộ
    if (isLocked) return;

    // Xóa timer cũ nếu người dùng cuộn liên tục
    clearTimeout(scrollTimeout);

    // Đặt timer mới: chỉ kích hoạt sau khi người dùng dừng cuộn 100ms
  ...
  const lockScroll = () => {
    ...
    setTimeout(() => {
      isLocked = false;
    }, ANIMATION_LOCK_MS); // khóa 600ms (bằng thời gian chuyển video)
  };
